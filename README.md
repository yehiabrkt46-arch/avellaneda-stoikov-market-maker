...
- Parity: the q edge decomposition reproduces the Python implementation on all 22 (strategy, day) buckets of the 9.66-day window to within 1e-6 USD (`scripts/run_kdb_parity.py`, exit 0). Full-window totals per strategy: avellaneda_stoikov spread capture -33.93 USD, adverse selection -56.39 USD over 33,538 fills; fixed_spread -13.73 and -52.08 USD over 27,198 fills.
- The aj-based independent recomputation (`scripts/run_kdb_verify.py`) surfaced a real property of the recorded feed rather than a bug: joining each fill to the last book tick at or before its timestamp reproduces the engine's stored fill-time mid for only 13% of fills, while joining to the first book tick at or after it matches 90.2% exactly. The trade channel's embedded exchange timestamps systematically lag the book channel by about one top-of-book update (median spacing 330 ms), and the engine correctly prices fills in arrival order, not embedded-timestamp order. The adverse-move recomputation agrees with the stored values at the median (p50 difference 0 against a median absolute move of 5.5 USD), with tail differences at feed gaps where the two observation conventions legitimately diverge.
- OFI study (`scripts/run_ofi_study.py`): on held-out days (same 8 train / 3 test chronological split as the parameter sweep), 1-second-bucketed order-flow imbalance predicts forward mid returns with out-of-sample R2 of 0.038 at 1 s, 0.037 at 5 s, and 0.025 at 10 s. Small but positive at every horizon, so the signal survives out of sample; it is not yet used by any strategy here, and wiring it into the A-S reservation price is the natural next experiment. The directional hit-rate metric is reported in `data/ofi-results.json` but is uninformative at this bucket size: about 90% of 1-second buckets have exactly zero mid change, so sign agreement is dominated by mechanical misses on no-move buckets rather than genuine directional errors.
- The sweep walk-forward report reruns in q (`scripts/run_kdb_report.py`) and reproduces the Python report exactly: same 12 candidates, same ranking, both winners identical with train and test totals agreeing to within 1e-6 USD.

## OFI overlay experiment

Since the OFI signal survived out of sample, the follow-up experiment wires it into the strategy: an online order-flow-imbalance estimator (same formula as the q study, computed incrementally from book updates over a trailing 1-second window) shifts the Avellaneda-Stoikov reservation price by the OFI-predicted forward mid move, using the beta fitted on training days only. Candidate skew scales are compared by replaying the full recorded window one candidate at a time (`scripts/run_ofi_overlay_sweep.py`), selection on the same 8 training days, and the held-out 3-day score of the selected candidate reported against an identically-run no-overlay control.

Result: the no-overlay control reproduced the M7 sweep's published scores to five decimal places (train -68.65578, test -19.42465), which doubles as an end-to-end determinism check on the new code path. The overlay candidate selected on training days alone (5-second-horizon beta, skew scale 1.0) improved the held-out test score from -19.42 to -8.19 USD, a 58 percent reduction in the conservative-floor loss, and every overlay candidate tested beat the control on both the training and the held-out days. The selected scale is an interior grid point (1.0 beat both 0.5 and 2.0 on train days), so this is not a search-boundary artifact. Honest limits: the score is still negative in absolute terms under the strict-cross fill floor, the fitted alpha term was dropped from the skew (a negligible constant bias, disclosed), and this is a replay result on one 9.66-day window, not a live run.
