...
Since the OFI signal survived out of sample, the follow-up experiment wires it into the strategy: an online order-flow-imbalance estimator (same formula as the q study, computed incrementally from book updates over a trailing 1-second window) shifts the Avellaneda-Stoikov reservation price by the OFI-predicted forward mid move, using the beta fitted on training days only. Candidate skew scales are compared by replaying the full recorded window one candidate at a time, selection on the same 8 training days, and the held-out 3-day score of the selected candidate reported against an identically-run no-overlay control. Status: running now; the held-out result lands here either way, improvement or not.
